...
function saveexpect(){	
	...
	layer.load('执行中，请稍候...',0);
	$.post("index.php?c=expect&act=saveexpect", { name: name, hy: hy, job_classid: job_classid, provinceid: provinceid, cityid: cityid, three_cityid: three_cityid, salary: salary, jobstatus: jobstatus, type: type, report: report, eid: eid, submit: "1", dom_sort: getDomSort() }, function (data) {
		layer.closeAll();
		if(data==0){
			layer.msg('操作失败！', 2, 8);
		}else if(data==1){
			...
			if(eid==""){
				...
				$("#eid").val(data.id);
				var html='<li><span>期望从事行业：</span>'+data.hy+'</li><li><span>期望工作地点：</span>'+data.city+'</li><li><span>期望薪资：</span>'+data.salary+'</li><li><span>到岗时间：</span>'+data.report+'</li><li><span>求职状态：</span>'+data.jobstatus+'</li><li><span>期望工作性质：</span>'+data.type+'</li><li  class="expect_amend_end"><span>期望从事职位：</span>'+data.job_classname+'</li>';
				$("#expect").html(html);
...
function savedescription(){
	...
	var eid = $.trim($("#eid").val());
	var description = $.trim($("#description").val()); 
	if(description==""){layer.msg('请填写自我评价！', 2, 2);return false; }
	layer.load('执行中，请稍候...',0);
...
function addexpect(){	
	...
	var eid = $.trim($("#eid").val());
	var jobstatus = $.trim($("#statusid").val());
	if(name==""){layer.msg('请填写简历名称！', 2, 2);return false; }
	if(hy==""){layer.msg('请选择从事行业！', 2, 2);return false;}
	if(three_cityid=="" && cityid==''){layer.msg('请选择工作地点！', 2, 2);return false;}
	if(job_classid==""){layer.msg('请选择从事职位！', 2, 2);return false;}
	if(salary==""){layer.msg('请选择期望薪资！', 2, 2);return false;}
