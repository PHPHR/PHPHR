	...
	public function resume($row){
		...
		if(@is_array($rows)){
			foreach($rows as $key=>$va){
				...
				if(strstr($row['code'],"{url}"))$data[$key]['url']=Url('resume',array("id"=>$va['id']));
	...
	public function member($row){		
		...
		if(is_array($rows)){
			foreach($rows as $key=>$va){			
				...
				if(strstr($row['code'],"{email}"))$data[$key]['email']=$va['email'];
				if(strstr($row['code'],"{url}"))$data[$key]['url']=Url("ask",array("c"=>'friend',"uid"=>$va['uid']));
	...
	public function ask($row){
		...
		if(is_array($rows)){
			foreach($rows as $key=>$va){
				...
				if(strstr($row['code'],"{user_url}"))$data[$key]['user_url']=Url("ask",array("c"=>'friend',"id"=>$va['uid']));
			}
		}
		$this->cache($row['id'],$data);
	}
