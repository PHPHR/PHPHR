...
//        $this->yun_tpl(array('index'));
        $this->yun_tpl(array('index_login'));
    }
        ...
        if($username!="")
        {
            ...
            if($this->config['sy_uc_type']=="uc_center")
            {
                ...
                if($uid<1)
                {
                    ...
                    if(!empty($user))
                    {
                        ...
                        if($this->config['user_status']=="1"){
                            if($user['email_status']!="1"){
                                $this->ajaxlogin("6.您的账户还未激活，请先激活!",Url("activate",'',"1"));
                                die;
                            }
                        }
                        if($_POST['loginname']){
                    ...
                    $this->ajaxlogin($ucsynlogin,Url("member"),'2');
                ...
                if(is_array($user)){
                    ...
                    if($user['password']==$pass)
                    {
                        if($user['status']=="2")
                        {
                            $this->ajaxlogin("11.您的账号已被锁定!",Url("register",array("c"=>"ok","type"=>2),"1"));
                        }
                        if($user['usertype']=="2" && $this->config['com_status']!="1"&&$user['status']!="1"){
                            $this->ajaxlogin("12.您还没有通过审核!",Url("register",array("c"=>"ok","type"=>3),"1"));
                        }
                        if($this->config['user_status']=="1" && $user['usertype']=="1"&&$user['email_status']!="1"){
                            $this->ajaxlogin("13.您的账户还未激活，请先激活！",Url("activate",'',"1"));
                        }
                        if (session_id() == ""){session_start();}
                        if($_SESSION['qq']['openid'])
                        {
                        ...
                        $this->ajaxlogin("14.密码不正确！");
                    }
                }else{
                    $this->ajaxlogin("15.该用户不存在！");
                }
            }
        }else{
            $this->ajaxlogin("16.用户名不能为空！");
        }
    }
