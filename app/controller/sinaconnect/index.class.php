...
class index_controller extends common
{
	function index_action()
	{
		...
		if($this->config['sy_sinalogin']!="1")
		{
			if($_GET['login']=="1")
			{
				$this->ACT_msg("index.php","对不起，新浪绑定已关闭！");
			}else{
				$this->ACT_msg("index.php","对不起，新浪登录已关闭！");
			}
		}
		...
		if(isset($_GET['code']))
		{
			...
			if($token['access_token'])
			{
				...
				if($tokenuid>0)
				{
					...
					{
						$this->obj->DB_update_all("member","`sinaid`=''","`qqid`='".$_SESSION['sina']["openid"]."'");
						$this->obj->DB_update_all("member","`sinaid`='".$_SESSION['sina']["openid"]."'","`uid`='".$this->uid."'");
						$this->ACT_msg($this->config['sy_weburl'].'/member/index.php?c=binding',"新浪微博登录绑定成功！",9);
					}
					...
					if(is_array($userinfo))
					{
						...
						if($user['name']){
							...
							$this->ACT_msg($this->config['sy_weburl'],"用户信息获取失败，请重新登陆新浪微博！",8);
						}
						header("location:".Url("sinaconnect",array("c"=>"sinabind")));
					}
				}else{
					$this->ACT_msg($this->config['sy_weburl'],"新浪微博授权失败，请重新授权！");
				}
			}
		}else{
	  ...
	  function sinabind_action()
	{
		...
		if(($_GET['usertype']=='1' || $_GET['usertype']=='2') && $_SESSION['sina']['openid'])
		{
				...
				$this->add_cookie($userid,$username,$salt,$email,$pass,$usertype);
 				unset($_SESSION['sina']);
				$this->ACT_msg($this->config['sy_weburl'].'/member',"登录成功！",9);
		}
		$this->seo("sinalogin");
