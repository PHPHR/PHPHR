...
//			var_dump($rows);die;
        ...
        //本人头像, 浏览者的
        if ($this->uid) {
            $member = $FriendM->GetFriendInfo(array("uid" => $this->uid), array("field" => 'pic,uid,usertype'));
            $this->yunset("member", $member);
        }
        ...
        //受访者
        if (!empty($_GET['uid'])) {
            $interviewee = $FriendM->GetFriendInfo(array("uid" => $_GET['uid']), array("field" => 'pic,uid,usertype'));
            $this->yunset("interviewee", $interviewee);
        }
        ...
        $this->yunset("navtype", 'myquestion');
        $this->yunset("uid", $uid);
//		$this->ask_tpl('message'); //旧模版
//die;
        if ($_COOKIE['uid'] == $_GET['uid'] || empty($_GET['uid'])) { //访问自己
            if(empty($_COOKIE['uid'])){
                header("Location: /?m=login"); //跳转到登录页面
            }
            if ($member['usertype'] == 2){
                $this->ask_tpl('message_3'); //如果访问用户为企业, 访问自己
            }else{
                $this->ask_tpl('message_1'); //如果访问用户为普通用户, 访问自己
            }
        } else {
            if ($interviewee['usertype'] == 2){
                $this->ask_tpl('message_4'); //如果受访问用户为企业, 访问他人
            }else{
                $this->ask_tpl('message_2'); //如果受访问用户为普通用户, 访问他人
            }
        }
