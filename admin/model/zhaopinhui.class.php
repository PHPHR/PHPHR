...
class zhaopinhui_controller extends common
{
	...
	function save_action(){
		...
		if($update){
			...
			if($info['did']!=$_POST['did']){
				$this->obj->DB_update_all("zhaopinhui_pic","`did`='".$_POST['did']."'","`id`='".$id."'");
				$this->obj->DB_update_all("zhaopinhui_com","`did`='".$_POST['did']."'","`id`='".$id."'");
			}
	...
	function uploadsave_action(){
		...
		if($add){
			...
			$_POST['is_themb']=0;
			$row=$this->obj->DB_select_once("zhaopinhui","`id`='".$zid."'");
			$_POST['did']=$row['did'];
		...
		if($update){
			...
			isset($nbid)?$this->ACT_layer_msg("招聘会图片(ID:".$id.")修改成功！",9,$_SERVER['HTTP_REFERER'],2,1):$this->ACT_layer_msg("修改失败！",8,$_SERVER['HTTP_REFERER']);
		}
	}
	function pic_action()
	{
