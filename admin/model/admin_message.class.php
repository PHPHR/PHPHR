...
class admin_message_controller extends common
{
	...
	function index_action()
	{
        ...
        $M=$this->MODEL();
		$PageInfo=$M->get_page("message",$where,$pageurl,$this->config['sy_listnum']);
   ...
   function show_action()
   {
		if($_POST['submit'])
		{
			...
			$nid=$this->obj->DB_update_all("message",$value,"`id`='".$_POST['id']."'");
 		    $nid?$this->ACT_layer_msg("留言反馈回复(ID:$nid)成功！",9,"index.php?m=admin_message",2,1):$this->ACT_layer_msg("回复(ID:$nid)失败！",8,$_SERVER['HTTP_REFERER']);
		}
