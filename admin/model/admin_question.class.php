...
class admin_question_controller extends common
{
	...
	function delquestion($ids=array()){ 
		...
		if(count($uid)){
			$friend=$this->obj->DB_select_all("friend_info","`uid` in(".pylode(',',$uid).")","`answer`,`uid`");	
			foreach($friend as $val){
				$answer=$val['answer']-$num[$val['uid']];
				if($answer<0){$answer='0';}
				$this->obj->DB_update_all('friend_info',"`answer`='".$answer."'","`uid`='".$val['uid']."'");
			}
		}
		foreach($ids as $v){
		...
		foreach($ask as $val){
			$this->obj->DB_update_all('friend_info',"`ask`=`ask`-'".$val['num']."'","`uid`='".$val['uid']."'");
		}
