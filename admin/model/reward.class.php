...
class reward_controller extends common{
	...
	function status_action()
	{
		...
		$this->admin_log("奖品(ID:".$_GET['id'].")状态设置成功！");
		echo $id?1:0;die;
	}
	...
	function group_action()
	{
		if($_POST['submit'])
		{
			...
			isset($id)?$this->ACT_layer_msg("奖品类别(ID:".$id.")添加成功！",9,"index.php?m=reward&c=group",2,1):$this->ACT_layer_msg("添加失败！",8,"index.php?m=reward&c=group");
		}
	...
	function ajax_action(){
		if($_POST['id']){
			...
			$nbid=$this->obj->DB_update_all("reward_class","`name`='".trim($_POST['name'])."'","`id`='".$_POST['id']."'");
            $this->admin_log("奖品类别(ID:".$nbid.")修改成功！");
            echo '1';die;
		}
	}
