	...
	function add_action(){
		if(!empty($_POST['add_name'])){
			if(!is_array($this->obj->DB_select_once("industry","`name`='".$_POST['add_name']."'"))){
				$add=$this->obj->DB_insert_once("industry","`name`='".iconv('utf-8','gbk',trim($_POST["add_name"]))."',`sort`='".$_POST['add_sort']."'");
				$this->cache_action();
			    $add?$msg=3:$msg=4;
			    $this->admin_log("行业类别(ID:".$add.")添加成功！");
			}else{
				$msg=2;
			}
		}else{
