 ...
 */
class advertise{
	function __construct($obj,$common=null){
		$this->obj = $obj;
        $this->common=$common;
	...
	function model_saveadd_action($post,$pic=NULL){
		...
		if($ad_type=="word")
		{
			...
			$this->common->ACT_layer_msg("您还未选择广告类型！",8,"index.php?m=advertise&c=ad_add");
		}
		$this->model_ad_arr_action();
		$this->common->ACT_layer_msg("广告添加成功！",9,"index.php?m=advertise",2,1);
	}
	function model_modify_save_action($post,$pic){
		...
		$this->model_ad_arr_action();
		$this->common->ACT_layer_msg("广告修改成功！",9,$lasturl,2,1);
	}
