...
class admin_company_controller extends common{
        ...
        $rows=$PageInfo['rows'];
 		if(is_array($rows)&&$rows){
			if($_GET['type']!='1'&&empty($username)){
				...
				$username=$this->obj->DB_select_all("member","`uid` in (".@implode(",",$uids).")","`username`,`uid`,`reg_date`,`login_date`,`status`,`source`");
			}
			if(empty($list)){
	...
	function rating_action(){
		...
		if(is_array($statis) && !empty($statis))
		{
			...
			$this->admin_log("企业会员(ID".$_POST['uid'].")更新会员等级");
		}else{
			$value="`uid`='".$_POST['uid']."',";
			$value.=$this->rating_info($ratingid);
	...
	function getstatis_action(){
		...
		if($_POST['uid'])
		{
			$rating	= $this->obj->DB_select_once("company_statis","`uid`='".intval($_POST['uid'])."'","`rating`,`lt_job_num`,`lt_down_resume`,`lt_editjob_num`,`lt_breakjob_num`,`job_num`,`down_resume`,`editjob_num`,`invite_resume`,`breakjob_num`,`msg_num`,`vip_etime`,`pay`,`integral`");
	...
	function getrating_action(){
		...
		if($_POST['id'])
		{
			$rating	= $this->obj->DB_select_once("company_rating","`id`='".intval($_POST['id'])."' and `category`='1'","`resume`,`job_num`,`interview`,`editjob_num`,`breakjob_num`,`service_time`,`coupon`,`category`,`type`");
	...
	function lock_action(){
		...
		$email=$this->obj->DB_select_once("company","`uid`='".$_POST['uid']."'","`email`,`name`");
		...
		if($this->config['sy_smtpserver']!="" && $this->config['sy_smtpemail']!="" && $this->config['sy_smtpuser']!=""){
			$this->send_msg_email(array("email"=>$email['email'],"uid"=>$_POST['uid'],"name"=>$email['name'],"lock_info"=>$_POST['lock_info'],"type"=>"lock"));
		}
	...
	function xls_action()
	{
		if($_POST['where'])
		{
			...
			$list=$this->obj->DB_select_all("company",$_POST['where'],$select);
			if(!empty($list))
			{
				if(in_array("rating",$gettype))
				{
					...
					$rating=$this->obj->DB_select_all("company_statis","uid in (".@implode(",",$uid).")","uid,rating_name");
					foreach($list as $k=>$v)
					{
						foreach($rating as $val)
						{
