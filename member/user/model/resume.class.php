...
class resume_controller extends user{
	function index_action(){
		...
		$rows=$this->obj->DB_select_all("resume_expect","`uid`='".$this->uid."' order by defaults desc","id,name,lastupdate,height_status,doc,tmpid,integrity,hits,statusbody,`topdate`");
	...
	function rtop_action(){
		...
		if($days<1){$this->ACT_layer_msg("请输入正确的置顶天数！",8);}
		if(intval($_POST['eid'])<1){$this->ACT_layer_msg("非法操作！",8);}
		...
		if($num>$statis['integral']){
			$this->ACT_layer_msg($this->config['integral_pricename']."不足，请减少置顶天数！",8);
		}else{
			...
			if($result){
				...
				$this->ACT_layer_msg("操作失败！",8,$_SERVER['HTTP_REFERER']);
			}
		}
	}
