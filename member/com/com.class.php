...
class company extends common
{
				...
				if($type==1){
					if($statis['job_num']>0){
						...
						if($this->config['com_integral_online']=="1"){
							...
							$this->ACT_layer_msg("会员发布职位用完,购买会员服务更快捷！",8,"index.php?c=job&w=1");
						}
					}
				}elseif($type==2){
					...
					if($statis['editjob_num']>0){
						...
						if($this->config['com_integral_online']=="1"){
							...
							$this->ACT_layer_msg("会员修改职位用完！",8,"index.php?c=job&w=1");
						}
					}
				}elseif($type==3){
					...
					if($statis['lt_job_num']>0){
						...
						if($this->config['com_integral_online']=="1"){
							...
							$this->ACT_layer_msg("会员发布高级职位用完，购买会员服务更快捷！",8,"index.php?c=job&w=1");
						}
					}
				}elseif($type==5){
					...
					if($statis['lt_breakjob_num']>0){
						$value="`lt_breakjob_num`=`lt_breakjob_num`-1";
					}else{
						if($this->config['com_integral_online']=="1"){
							$this->intergal($type,$statis);
						}else{
							$this->ACT_layer_msg("会员刷新高级职位用完！",8,"index.php?c=job&w=1");
						}
					}
				}
				if($value){
	...
	function intergal($type,$statis)
	{
		if($type==1 && $this->config['integral_job']){
		...
		}elseif($type==4 && $this->config['integral_lt_job']){
			if($this->config['integral_lt_job_type']=="1")
			{
				$auto=true;
			}else{
				if($statis['integral']<$this->config['integral_lt_job']){
					$this->ACT_layer_msg("你的".$this->config['integral_pricename']."不够发布高级职位！",8,"index.php?c=pay");
				}
				$auto=false;
			}
			$nid=$this->company_invtal($this->uid,$this->config['integral_lt_job'],$auto,"发布职位",true,2,'integral',18);
		}elseif($type==5 && $this->config['integral_lt_jobedit']){
			if($this->config['integral_lt_jobedit_type']=="1")
			{
				$auto=true;
			}else{
			...
			$nid=$this->company_invtal($this->uid,$this->config['integral_lt_jobedit'],$auto,"修改职位",true,2,'integral',19);
		}
	}
	...
	function get_user(){
		...
		if(!$rows['name'] || !$rows['address'] || !$rows['pr']){
			$this->ACT_msg("index.php?c=info","请先完善公司资料！");
		}
		return $rows;
	}
	function job(){
		...
		if($_POST['urgentid'])
		{
			...
			if($reow['integral']<$integral && $this->config['com_urgent_type']=="2")
			{
				...
				$this->obj->member_log("发布紧急职位《".$job['name']."》",1,1);
 				$this->ACT_layer_msg("发布紧急职位成功！",9,$_SERVER['HTTP_REFERER']);
			}
		}
