...
<div class="main">
    <textarea class="pact" readonly="readonly">
        安装协议： 本授权协议适用且仅适用于PHPYUN.4.0 版本，宿迁鑫潮信息技术有限公司拥有对本授权协议的最终解释权。 I. 协议许可的权利
        1. 您可以在完全遵守本最终用户授权协议的基础上，将本软件应用于非商业用途(包括个人用户：不具备法人资格的自然人，以个人名义从事网络威客交易；非盈利性用途：从事非盈利活动的商业机构及非盈利性组织，将PHPYUN
        人才系统仅用于产品演示、展示及发布，而并不是用来买卖及盈利的运营活动的) 2. 您可以在协议规定的约束和限制范围内修改 PHPYUN人才网系统
        源代码(如果被提供的话)或界面风格以适应您的网站要求。 3. 您拥有使用本软件构建的人才系统中全部招聘信息，求职，用户信息及相关信息的所有权，并独立承担与其内容的相关法律义务。
        ...
        II. 协议规定的约束和限制 1. 未获商业授权之前，不得将本软件用于商业用途(包括但不限于企业法人经营的企业网站、经营性网站、以盈利为目或实现盈利的网站)。
        2. 不得对本软件或与之关联的商业授权进行出租、出售、抵押或发放子许可证。 3. 无论如何，即无论用途如何、是否经过修改或美化、修改程度如何，只要使用PHPYUN
        人才系统 的整体或任何部分，未经书面许可，网站标题的Powered by PHPYun.都必须保留，而不能清除或修改。 4. 如果您未能遵守本协议的条款，您的授权将被终止，所被许可的权利将被收回，并承担相应法律责任。
        III. 有限担保和免责声明 1. 本软件及所附带的文件是作为不提供任何明确的或隐含的赔偿或担保的形式提供的。 2. 用户出于自愿而使用本软件，您必须了解使用本软件的风险，在尚未购买产品技术服务之前，我们不承诺提供任何形式的技术支持、使用担保，也不承担任何因使用本软件而产生问题的相关责任。
        3. 宿迁鑫潮信息技术有限公司不对使用本软件构建的人才系统中的文章或任务信息承担责任，但在不侵犯用户隐私信息的前提下，保留以任何方式获取用户及商品信息的权利。
        有关 phpyun人才网系统! 最终用户授权协议、商业授权与技术服务的详细内容，均由PHPYUN 官方网站独家提供。 宿迁鑫潮信息技术有限公司拥有在不事先通知的情况下，修改授权协议和服务价目表的权力，修改后的协议或价目表对自改变之日起的新授权用户生效。电子文本形式的授权协议如同双方书面签署的协议一样，具有完全的和等同的法律效力。您一旦开始安装
        PHPYUN4.0，即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。协议许可范围以外的行为，将直接违反本授权协议并构成侵权，我们有权随时终止授权，责令停止损害，并保留追究相关责任的权力。
    </textarea>
</div>
<div class="bottom">
            ...
            <table width="100%">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <input class="input" name="dbname" type="text" value="phpyun">
                        </td>
                        <td>
...
<div class="bottom">
	...
	//判断提交数据
	 echo Encode('<script>document.getElementById("kays").innerHTML+="<dd>'.$dbname_states.'数据库 $dbname...成功</dd>;document.getElementById("kays").scrollTop+='.(30).';</script>','gbk');  
	sleep(1);
    //设定编码
    ...
    foreach($arrdata[0] as $k=>$v){
        ...
        echo Encode('<script>document.getElementById("kays").innerHTML+="<div>正在建表'.$tablearr[1][0].'...成功</div>";document.getElementById("kays").scrollTop+='.(30).';</script>','gbk');  
        ob_flush();
        flush();  
        ob_end_clean();
    }
    ...
    foreach($data[0] as $k=>$v){
        echo Encode('<script>document.getElementById("kays").innerHTML+="<div>正在导入数据'.$k.'的第'.($k+1).'条记录</div>";document.getElementById("kays").scrollTop+='.(30).';</script>','gbk');  
        ob_flush();
        flush();  
        ob_end_clean();
        $sql=str_replace("\r\n","",$v);
    ...
    mysql_query("INSERT INTO $table_user SET `uid`='1',`m_id`='1',`username`='".$username."',`password`='".$password."',`name`='超级管理员',`did`='0',`lasttime`='".time()."'");
   ...
   for($i=0;$i<10;$i++){
    ...
    echo Encode('<script>document.getElementById("kays").innerHTML+="<div>添加管理员...成功</div>";document.getElementById("kays").scrollTop+='.(30).';</script>','gbk');  
    ...
    fclose($config);
    mysql_close($conn);
        echo Encode('<script>document.getElementById("kays").innerHTML+="<div>生成系统文件...成功</div>";document.getElementById("kays").scrollTop+='.(30).';</script>','gbk');  

    echo Encode('<script>document.getElementById("kays").innerHTML+="<div>初始化数据...成功</div>";document.getElementById("kays").scrollTop+='.(30).';</script>','gbk');  

    echo Encode('<script>document.getElementById("kays").innerHTML+="<div>更新网站缓存...成功</div>";document.getElementById("kays").scrollTop+='.(30).';</script>','gbk');  
...
<div class="step">
    ...
    <div class="server_2">
        <div class="info">
            ...
            <p>
                PHPYUN人才系统致力于帮助站长提高网站流量，增强网站运营能力，增加网站收入。
            </p>
            <p>
                PHPYUN人才系统目前免费提供了QQ互联、整合UC、多套模板、邮件提醒等功能。PHPYUN人才系统将陆续提供更多优质的服务项目。
            </p>
            <p style="color:#999;">
                大家在使用过程中，可以通过论坛向我们反馈意见及BUG。
                <a href="http://bbs.phpyun.com" target="_blank">
                    论坛地址
                </a>
            </p>
            <br>
