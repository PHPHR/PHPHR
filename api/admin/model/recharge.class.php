...
{
	function getmember_action()//查询用户
	{
		if(!$_POST['usertype'])
		{
		...
		{	
			...
			if($_POST['usertype']=='2'){				
				$rating_rows=$this->obj->DB_select_all("company_statis","`uid` in(".pylode(',',$uids).")",'`rating_name`,`rating`,`uid`');
			}
			foreach($rows as $k=>$v)
			{
	...
	function recharge_action()//会员充值
	{
		...
		foreach($uids as $v)
		{
			...
			$value.="`uid`='".$v."'";
			$this->obj->DB_insert_once("company_order",$value);//充值记录
		}
		if($_POST['usertype']=="1")
		{
	...
	function getrating_action()//获取会员等级
	{
		if(!$_POST['usertype'])
		{
			$this->return_appadmin_msg(2,"参数出错");
