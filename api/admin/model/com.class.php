...
class com_controller extends appadmin
{
	...
	function jobstatus_action()//职位审核
	{
	...
	function jobshow_action()//职位详情
	{
		...
		if(!empty($row))
		{
			...
			$list['comid']	=$row['uid'];
			$list['comname']=iconv("gbk","UTF-8",$row['com_name']);//公司名称com_name
			$list['hy']		=$row['hy'];
			$list['job1']	=$row['job1'];
	...
	function certlist_action()//企业认证列表
	{
	...
	function certstatus_action()//企业认证审核
	{
	...
	function certdel_action()//企业认证删除
	{
		if(!$_POST['ids'])
		{
			$this->return_appadmin_msg(2,"参数出错");
	...
	function newslist_action()//企业新闻列表
	{
		$where="a.uid=b.uid";
		$page=$_POST['page'];
		$limit=$_POST['limit'];
	...
	function newsshow_action()//企业新闻详情
	{
		if(!$_POST['id'])
		{
			$this->return_appadmin_msg(2,"参数出错");
	...
	function newsdel_action()//企业新闻删除
	{
		if(!$_POST['ids'])
		{
			$this->return_appadmin_msg(2,"参数出错");
	...
	function newsstatus_action()//企业新闻审核
	{
	...
	function productlist_action()//企业产品列表
	{
		$where="a.uid=b.uid";
		$page=$_POST['page'];
		$limit=$_POST['limit'];
	...
	function productshow_action()//企业新闻详情
	{
		if(!$_POST['id'])
		{
			$this->return_appadmin_msg(2,"参数出错");
	...
	function productdel_action()//企业产品删除
	{
		if(!$_POST['ids'])
		{
			$this->return_appadmin_msg(2,"参数出错");
	...
	function productstatus_action()//企业产品审核
	{
		if(!$_POST['ids']||!$_POST['status'])
		{
			$this->return_appadmin_msg(2,"参数出错");
	...
	function oncelist_action()//微招聘列表
	{
		$where=1;
		$page=$_POST['page'];
		$limit=$_POST['limit'];
	...
	function onceshow_action()//微招聘详情
	{
		if(!$_POST['id'])
		{
			$this->return_appadmin_msg(2,"参数出错");
	...
	function oncestatus_action()//微招聘审核
	{
		if(!$_POST['ids']||!$_POST['status'])
		{
			$this->return_appadmin_msg(2,"参数出错");
	...
	function companylist_action()//公司管理列表
	{
		$where="a.uid=b.uid";
		$page=$_POST['page'];
		$limit=$_POST['limit'];
		...
		if(!empty($rows))
		{
			...
			$statis=$this->obj->DB_select_all("company_statis","`uid` in (".pylode(",",$uid).")","`uid`,`rating_name`");
			foreach($rows as $k=>$v)
			{
	...
	function companyshow_action()//企业详情
	{
		if(!$_POST['uid'])
		{
			$this->return_appadmin_msg(2,"参数出错");
	...
	function companydel_action()//企业删除
	{
		if(!$_POST['uids'])
		{
			$this->return_appadmin_msg(2,"参数出错");
	...
	function companystatus_action()//企业审核
	{
